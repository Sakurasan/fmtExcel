...
func (this *MainController) Post() {
	...
	fe := this.Ctx.Request.MultipartForm.File["excel"][0]

	if fe == nil {
		fmt.Println("文件信息上送不能为空")
		return
	}

	f, err := fe.Open()
	if err != nil {
		fmt.Println("打开上传文件失败" + err.Error())
		return
	}
	defer f.Close()
	...
	if err != nil {
		fmt.Println("读取上传文件失败" + err.Error())
		return
	}

	xf, err := xlsx.OpenBinary(fb)
	if err != nil {
		fmt.Println("加载文件失败，" + err.Error())
		return
	}

	nsc, _ := strconv.Atoi(cate)
	nnc, _ := strconv.Atoi(namecate)
	//deal xlsx
	sht1 := xf.Sheets[0]
	headRow := sht1.Rows[0]

	resMap := make(map[string]*xlsx.File)
	fnMap := make(map[string]string)

	idx := 0
	for _, r := range sht1.Rows[1:] {
		nameCel, _ := r.Cells[nnc].String()
		selCel, _ := r.Cells[nsc].String()
		if _, ok := fnMap[selCel]; !ok {
			fnMap[selCel] = nameCel
			resMap[selCel] = xlsx.NewFile()
			st, _ := resMap[selCel].AddSheet("sheet1")
			//head 要单独处理  扩列
			dr := st.AddRow()
			for _, c := range headRow.Cells {
				dr.AddCell().SetString(c.Value)
			}
		}
		resMap[selCel].Sheets[0].Rows = append(resMap[selCel].Sheets[0].Rows, r)
		idx++
	}
	fmt.Println("文件拆分完成", idx)
	//zip
	rb := &bytes.Buffer{}
	rz := zip.NewWriter(rb)
	for k, v := range fnMap {
		//u2g := mahonia.NewEncoder("UTF8")
		//dst := []byte(u2g.ConvertString(v))
		//header := &zip.FileHeader{
		//	Name:   v + ".xlsx",
		//	Flags:  1 << 11, // 使用utf8编码
		//	Method: zip.Deflate,
		//}
		//fio, err := rz.CreateHeader(header)
		fio, err := rz.Create(v + ".xlsx")
		if err != nil {
			fmt.Println("压缩创建文件失败", k, err)
			break
		}
		fmt.Println("deal k", k, v, resMap[k])
		err = resMap[k].Write(fio)
		if err != nil {
			fmt.Println("写文件失败", k, err)
			break
		}
	}
	rz.Close()

	desFileName := "static/ZipFile/" + time.Now().Format("20060102150405") + ".zip"

	fres, err := os.OpenFile(desFileName, os.O_CREATE|os.O_WRONLY, 0755)
	rb.WriteTo(fres)
	fres.Close()
	...
	// 	fmt.Println(err)
	// }
	// this.Redirect(desFileName, 302)
	this.Data["Url"] = desFileName
}
